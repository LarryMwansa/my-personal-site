...
const themes = [
  { value: "system", label: <FaDesktop title="System" /> },
  { value: "light", label: <FaSun title="Light" /> },
  { value: "dark", label: <FaMoon title="Dark" /> },
];
...
function ThemeToggle() {
  ...
  if (!mounted) {
    return <div className={styles.themeToggle}><div className={styles.themeSelect} style={{width: '90px', height: '35px'}}></div></div>;
  }

  return (
    <div className={styles.themeToggle}>
      <div className={styles.themeToggleGroup} role="group" aria-label="Theme mode">
        {themes.map(opt => (
          <button
            key={opt.value}
            type="button"
            className={
              theme === opt.value
                ? `${styles.themeToggleBtn} ${styles.themeToggleBtnActive}`
